	...
	<div class="">
		...
		<div class="row">
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="x_panel">
					<div class="x_content">
						<form class="form-horizontal form-label-left" action="{{ url('member/activities/'.$activities->id) }}" method="POST" novalidate>
							{!! csrf_field() !!}
							{{ method_field('PUT') }}
							...
							<div class="item form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<textarea id="input-description" class="form-control col-md-7 col-xs-12" name="description" required="required">{{ old('name', $activities->description) }}</textarea>
								</div>
							</div>

							<div class="item form-group">
								<label class="control-label col-md-3 col-sm-3 col-xs-12" for="input-description">
									ประเภทกิจกรรม{{ $activities->category_id}} <span class="required">*</span>
								</label>
								<div class="col-md-6 col-sm-6 col-xs-12">
									<select class="selectpicker" name="category">
										@foreach(App\ActivitiesCategory::all() as $value)
											<option value="{{ $value->id }}"{!! old('category', $activities->category_id) == $value->id ? ' selected="selected"':'' !!}>{{ $value->name }}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="item form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<select class="selectpicker" name="term">
										<option value="1"{!! old('term', $activities->term) == 1 ? ' selected="selected"':'' !!}>1</option>
										<option value="2"{!! old('term', $activities->term) == 2 ? ' selected="selected"':'' !!}>2</option>
										<option value="3"{!! old('term', $activities->term) == 3 ? ' selected="selected"':'' !!}>3</option>
									</select>
								</div>
							</div>
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<select class="selectpicker" name="year">
										@for($i=(date('Y')+10); $i>(date('Y')-5); $i--)
											<option value="{{ $i }}"{!! old('year', $activities->year) == $i ? ' selected="selected"':'' !!}> {{ $i+543 }}</option>
										@endfor
									</select>
								</div>
							</div>
							...
							<div class="item form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<input type="text" style="width: 200px" name="event-start" id="event-start" class="form-control" required="required" value="{{ old('event-start', new Carbon\Carbon($activities->event_at))->format('d/m/Y')  }}" />
								</div>
							</div>
							...
							<div class="item form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<select class="selectpicker" name="generation">
										<option value="0"{!! old('generation', $activities->generation) == 0 ? ' selected':'' !!}>ทุกรุ่น</option>
										@for($i=date('y')+44; $i>=date('y')+39; $i--)
											<option value="{{ $i }}"{!! old('generation', $activities->generation) == $i ? ' selected':'' !!}>{{ $i }}</option>
										@endfor
									</select>
								</div>
							</div>
							...
							<div class="item form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<select class="selectpicker" name="section">
                                        ...
                                        <option value="a1"{!! old('section', $activities->section) == 'a1' ? ' selected':'' !!}>A1</option>
                                        <option value="b1"{!! old('section', $activities->section) == 'b1' ? ' selected':'' !!}>B1</option>
                                        <option value="c1"{!! old('section', $activities->section) == 'c1' ? ' selected':'' !!}>C1</option>
									</select>
								</div>
							</div>
